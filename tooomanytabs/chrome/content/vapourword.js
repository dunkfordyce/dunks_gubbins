...
vapourword.defaults = {
    ...
    min_font_size: 10,
    // items that will be added in each chunk
    add_chunk_size: 10,
    // time to wait between adding each chunk
    add_chunk_time: 200,
    // amount of placement fails before giving up - controlls endless searching 
    max_placement_fails: 10,
    // amount of times an edge will be tested before it is removed 
    // from the edge list. Lower numbers will mean less quality
    max_edge_fails: 10,
    ...
    this.add = function(word) {
        ...
        while(true) {
            ...
            if( edge.n[1] == -1.0 ) {
                ...
                ddd('didnt match normal', edge, edge.n);
            }
    ...
    function add_chunk(cloud) {
        ...
        while( i < start_index +cloud.options.add_chunk_size && i < items.length ) {
            ...
            if( !(item.added = cloud.add(item)) ) {
                cloud.fails ++;   
                if( cloud.fails > cloud.options.max_placement_fails ) {
                    stop = true;
                }
            }
